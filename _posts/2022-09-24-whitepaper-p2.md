...
Now before we can decide what parameters are the best for the weight distribution, we are going to build our intuition on the behavior of this ANN. Let's assume first that our parameters are $\mu=-0.5$ and $\sigma=0.1$ (see picture of the histogram above). In that cas all our weights $W$ are negatives, or inhibitory. Now this