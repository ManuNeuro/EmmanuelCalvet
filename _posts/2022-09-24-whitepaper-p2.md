...
In theory, the percolation threshold is defined as the point in parameter space where the perturbation can propagate at infinity. Here we have $100$ layers, if our goal was to know with precision the position of this threshold, we could increase the number of hidden layers. For our purpose, $100$ is long enough, since we only will transit by that point, and seek out another type of dynamic. As such, we will assume that the percolation threshold is reached as soon as the output is non-zero.
